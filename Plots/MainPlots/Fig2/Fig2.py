...
# Noise strengths
Ω       = 1e+1
Ωarray  = np.logspace(0,1.75,1000)
Ωarray *= Ω
...
for indo,Ωi in enumerate(Ωarray):
    for indr in range(len(actions_fw)):
        if indr < 5:
            fw_pre[indr,indo] = convert(fw_pre[indr,indo],actions_fw[indr],Ω,Ωi,interface='droplet')
        else:
            fw_pre[indr,indo] = convert(fw_pre[indr,indo],actions_fw[indr],Ω,Ωi,interface=False) 
    for indr in range(len(actions_bw)):
        if indr < 2:
            bw_pre[indr,indo] = convert(bw_pre[indr,indo],actions_bw[indr],Ω,Ωi,interface='droplet')
        else:
...
instpFW = instpFW.reshape(N+1,dofx,dofy)
instpFW = np.roll(instpFW,dofx//2,axis=1)
instpFW = np.roll(instpFW,dofy//2,axis=2)
...
instpBW = instpBW.reshape(N+1,dofx,dofy)
...
top = gs[0, 0:6].subgridspec(nrows=1, ncols=2, width_ratios=[1.0, 1.0], wspace=0.4)
...
cbar = fig.colorbar(contour_plot,ax=ax1,ticks=[0.25, 0.75], 
                    format=plt.FuncFormatter(lambda x, pos: r'$\rho_{+}$' if x > 0.5 else r'$\rho_{-}$'))
...
ax2.plot(fw_all[:,0]  , -fw_p, '-o', color=cm_blue ,label=r'$\rho_{-} \to \rho_{+}$', lw=4.0, markersize=10, zorder=3)	
ax2.plot(bw_all[:,0]  , -bw_p, '-o', color=cm_red  ,label=r'$\rho_{+} \to \rho_{-}$', lw=4.0, markersize=10, zorder=3)	

ax2.set_xlabel('$\kappa$', size=24,labelpad=8)
ax2.set_ylabel('$-S$', size=24, labelpad=12)

ax2.legend(ncol=1,loc='upper left', bbox_to_anchor=(0.3,1.0),fancybox=True,shadow=True)

ax2.set_xlim(np.min(fw_all[:,0]),4)
ax2.set_ylim(-3.4,-0.8)
...
ax2.text(0.7, -3.2, r"$\rho_{+}$ stable",fontsize=24,color='k')
ax2.text(2.2, -3.2, r"$\rho_{-}$ stable",fontsize=24,color='k')
...
# Plot each panel
idxsFW = [0, 3*N//16, N//2, 13*N//16, -1]
idxsBW = [0, 3*N//16, N//2, 12*N//16, -1]
 
for inda, idx in enumerate(idxsFW):
    # original grid
    X, Y = np.meshgrid(x[:-1], y[:-1], indexing='ij')
    Z = instpFW[idx]
    # finer grid
    xf = np.linspace(x[:-1].min(), x[:-1].max(), 4*len(x))
    yf = np.linspace(y[:-1].min(), y[:-1].max(), 4*len(y))
...
for inda, idx in enumerate(idxsBW):
    # original grid
    X, Y = np.meshgrid(x[:-1], y[:-1], indexing='ij')
    Z = instpBW[idx]
    # finer grid
...
cbar = fig.colorbar(contX, cax=caxX, ticks=np.linspace(vmin, vmax, 3))
cbar = fig.colorbar(contY, cax=caxY, ticks=np.linspace(vmin, vmax, 3))

# Match the colorbar height to one subplot
fig.canvas.draw() 
...
plt.savefig("2DSchloegl_Combine.pdf", format='pdf',  bbox_inches="tight", dpi=600)
